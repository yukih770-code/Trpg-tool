...
const mapEvents: RoomMapEvent[] = [
  { mapEventId: 'map-1', roomId: room.identity.roomId, mapId: 'map', seq: 1, createdAt: now, authorMemberId: host.memberId, eventKind: 'map.token_added', payload: { token: { id: 'hero', name: 'Hero', displayName: 'Hero', x: 20, y: 20, size: 'medium', footprint: { schemaVersion: 1, shape: 'axis-aligned-rectangle', coordinateSpace: 'scene-world', anchor: 'center', width: 1.5, height: 2 }, sourceType: 'roomActorBinding', kind: 'playerCharacter', actorBindingId: 'binding-hero', roomMemberId: joinedPlayer.memberId, hpSummary: { current: 14, max: 20 }, conditionSummary: ['Blessed'] } } },
...
const playerHero = playerMap.find((event) => event.eventKind === 'map.token_added' && (event.payload.token as { id?: string } | undefined)?.id === 'hero')?.payload.token as { hpDisplay?: { kind?: string; current?: number }; acDisplay?: { kind?: string }; actorBindingId?: string; footprint?: { width?: number; height?: number } } | undefined;
...
expect(playerHero?.footprint?.width === 1.5 && playerHero.footprint.height === 2, 'visible occupied geometry is projected with the visible Token');
