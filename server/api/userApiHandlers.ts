...
import {
  ...
  type PostgresUserProfileVisibility,
  type PostgresUserRecord,
  type PostgresUserRepository,
  type PostgresUserRepositoryErrorKind,
  type PostgresUserRepositoryResult,
} from '../adapters/postgresUserRepository.js';
import { errorResponse, okResponse, type ServerApiResponse } from './apiResponse.js';

export interface UserApiRequestContext {
  requestId?: string;
}

export interface GetUserByIdHandlerInput extends UserApiRequestContext {
  userId?: unknown;
}

export interface GetUserProfileHandlerInput extends UserApiRequestContext {
  userId?: unknown;
}

export interface GetUserByIdentityHandlerInput extends UserApiRequestContext {
  providerKind?: unknown;
  providerSubject?: unknown;
}

export interface SaveUserProfileHandlerInput extends UserApiRequestContext {
  profile?: unknown;
}

export interface UserApiHandlers {
  getUserByIdHandler(
    input: GetUserByIdHandlerInput,
  ): Promise<ServerApiResponse<PostgresUserRecord>>;
  getUserProfileHandler(
    input: GetUserProfileHandlerInput,
  ): Promise<ServerApiResponse<PostgresUserProfile>>;
  getUserByIdentityHandler(
    input: GetUserByIdentityHandlerInput,
  ): Promise<ServerApiResponse<PostgresUserRecord>>;
  /**
   * Internal-only skeleton for the future profile write route. It is exported
   * for service tests/composition, but this slice does not mount a public write
   * endpoint.
   */
  saveUserProfileHandler(
    input: SaveUserProfileHandlerInput,
  ): Promise<ServerApiResponse<PostgresUserProfile>>;
}

export interface CreateUserApiHandlersOptions {
  userRepository?: PostgresUserRepository;
}

type PlainRecord = Record<string, unknown>;

const PROFILE_VISIBILITIES = new Set<PostgresUserProfileVisibility>([
  'private',
  'campaignOnly',
  'unlisted',
  'public',
...
function translateRepositoryError<T>(
  error: { kind: PostgresUserRepositoryErrorKind; retryable?: boolean },
  requestId?: string,
): ServerApiResponse<T> {
...
function requiredStringError<T>(fieldName: string, requestId?: string): ServerApiResponse<T> {
  return errorResponse(
    400,
    { kind: 'bad_request', message: `${fieldName} must be a non-empty string.` },
    { requestId },
  );
}

function readProfilePayload(profile: unknown): PostgresUserProfile | ServerApiResponse<PostgresUserProfile> {
  ...
  if (!userId) return requiredStringError('profile.userId');
  if (!handle) return requiredStringError('profile.handle');
  if (!displayName) return requiredStringError('profile.displayName');

  const visibility = readNonEmptyString(profile.visibility);
  if (visibility !== undefined && !PROFILE_VISIBILITIES.has(visibility as PostgresUserProfileVisibility)) {
  ...
  return {
    ...
    visibility: (visibility as PostgresUserProfileVisibility | undefined) ?? 'private',
    pinned: [],
    sectionVisibility: isPlainRecord(profile.sectionVisibility) ? profile.sectionVisibility : {},
  };
}

function isApiError<T>(value: PostgresUserProfile | ServerApiResponse<T>): value is ServerApiResponse<T> {
  return isPlainRecord(value) && value.ok === false;
}

async function unwrapRepositoryResult<T>(
  result: PostgresUserRepositoryResult<T | null>,
  requestId: string | undefined,
  notFoundMessage: string,
): Promise<ServerApiResponse<T>> {
  if (result.ok === false) return translateRepositoryError(result.error, requestId);
...
export function createUserApiHandlers(
  options: CreateUserApiHandlersOptions = {},
): UserApiHandlers {
  ...
  return {
    async getUserByIdHandler(input) {
      ...
      if (!userId) return requiredStringError('userId', input.requestId);
    ...
    async getUserByIdentityHandler(input) {
      ...
      if (!providerKind) return requiredStringError('providerKind', input.requestId);
      const providerSubject = readNonEmptyString(input.providerSubject);
      if (!providerSubject) return requiredStringError('providerSubject', input.requestId);
      return unwrapRepositoryResult(
        await userRepository.getUserByIdentity(providerKind, providerSubject),
        input.requestId,
    ...
    async saveUserProfileHandler(input) {
      ...
      if (result.ok === false) return translateRepositoryError(result.error, input.requestId);
