...
export type PostgresSchemaReadinessStatus =
  | 'not_configured'
  | 'unreachable'
  | 'schema_missing'
  | 'ready'
...
export interface PostgresMissingColumns {
  table: string;
  columns: string[];
}

export interface PostgresSchemaReadinessResult {
  status: PostgresSchemaReadinessStatus;
  missingTables?: string[];
  missingColumns?: PostgresMissingColumns[];
  errorKind?: string;
  latencyMs?: number;
}
...
const REQUIRED_USER_SCHEMA_COLUMNS: Record<string, string[]> = {
...
function classifySchemaError(error: unknown): Pick<PostgresSchemaReadinessResult, 'status' | 'errorKind'> {
...
export async function checkPostgresUserSchemaReadiness(): Promise<PostgresSchemaReadinessResult> {
...
export function getRequiredPostgresUserSchemaColumns(): Record<string, string[]> {
