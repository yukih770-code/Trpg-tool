...
import {
  ...
  type PostgresActorRepositoryExecutor,
...
/**
 * Rollback-only Actor write smoke (P5.12C). Inside a single transaction it creates
 * a deterministic smoke OWNER user (reusing PostgresUserRepository with the same
 * client executor), then create/read-by-id/read-by-origin/list/update/archive/
 * restore a smoke actor, and ALWAYS rolls back. No permanent rows, no commit path,
 * no raw DB error or connection string in the output.
 */

export type PostgresActorWriteSmokeStatus =
  | 'not_configured'
  | 'unreachable'
  | 'user_schema_missing'
  | 'schema_missing'
...
export interface PostgresActorWriteSmokeStep {
  name: string;
  ok: boolean;
  errorKind?: string;
}

export interface PostgresActorWriteSmokeResult {
  status: PostgresActorWriteSmokeStatus;
  ...
  steps: PostgresActorWriteSmokeStep[];
  errorKind?: string;
}
...
function makeClientExecutor(
  clientQuery: PostgresActorRepositoryExecutor['query'],
): PostgresActorRepositoryExecutor {
  return {
    query: <T extends QueryResultRow>(text: string, values?: readonly unknown[]) => clientQuery<T>(text, values),
  };
}

function failedStep(name: string, errorKind: string): PostgresActorWriteSmokeStep {
  return { name, ok: false, errorKind };
}

async function runWritePath(
  executor: PostgresActorRepositoryExecutor,
): Promise<PostgresActorWriteSmokeStep[]> {
  const steps: PostgresActorWriteSmokeStep[] = [];
  ...
  if (created.ok === false) return [...steps, failedStep('createActor', created.error.kind)];
  steps.push({ name: 'createActor', ok: true });

  const byId = await actorRepository.getActorById(SMOKE_ACTOR_ID);
  if (byId.ok === false) return [...steps, failedStep('getActorById', byId.error.kind)];
  ...
  const byOrigin = await actorRepository.getActorByOrigin(
    SMOKE_OWNER_USER_ID,
    SMOKE_ACTOR_SYSTEM_ID,
    SMOKE_ACTOR_LOCAL_ID,
  );
  if (byOrigin.ok === false) return [...steps, failedStep('getActorByOrigin', byOrigin.error.kind)];
  ...
  if (listed.ok === false) return [...steps, failedStep('listActorsByOwner', listed.error.kind)];
  ...
  if (updated.ok === false) return [...steps, failedStep('updateActor', updated.error.kind)];
  ...
  if (archived.ok === false) return [...steps, failedStep('archiveActor', archived.error.kind)];
  ...
  if (restored.ok === false) return [...steps, failedStep('restoreActor', restored.error.kind)];
...
export async function runPostgresActorRepositoryRollbackWriteSmoke(): Promise<PostgresActorWriteSmokeResult> {
  ...
  if (schema.status !== 'ready') {
    const status: PostgresActorWriteSmokeStatus =
