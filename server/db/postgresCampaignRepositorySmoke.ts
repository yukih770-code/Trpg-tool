...
/**
 * Read-only Campaign repository smoke (P5.11B). Mirrors the User read smoke:
...
export type PostgresCampaignRepositorySmokeStatus =
  | 'not_configured'
  | 'unreachable'
  | 'user_schema_missing'
  | 'schema_missing'
...
export interface PostgresCampaignRepositorySmokeResult {
  status: PostgresCampaignRepositorySmokeStatus;
...
export async function runPostgresCampaignRepositoryReadOnlySmoke(): Promise<PostgresCampaignRepositorySmokeResult> {
