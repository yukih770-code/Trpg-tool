...
import {
  ...
  type PostgresRuntimeEventRepositoryExecutor,
...
/**
 * Rollback-only RuntimeEvent append smoke (P5.14C). Inside a single transaction it
 * creates smoke owner user + campaign + actor + runtime session, appends events
 * (verifying seq 1 -> 2), appends an idempotency-keyed event and retries the same
 * key (verifying safe de-duplication), reads/lists via afterSeq, then ends /
 * archives / restores the session, and ALWAYS rolls back. No permanent rows, no
 * commit path, no live runtime write, no WebSocket, no raw DB error/connection
 * string in the output. Because it uses `useInternalTransactions: false`, every
 * append runs on the outer transaction's client — never a nested connection.
 */

export type PostgresRuntimeEventWriteSmokeStatus =
  | 'not_configured'
  | 'unreachable'
  | 'user_schema_missing'
...
export interface PostgresRuntimeEventWriteSmokeStep {
  name: string;
  ok: boolean;
  errorKind?: string;
}

export interface PostgresRuntimeEventWriteSmokeResult {
  status: PostgresRuntimeEventWriteSmokeStatus;
  ...
  steps: PostgresRuntimeEventWriteSmokeStep[];
  errorKind?: string;
}
...
function makeClientExecutor(
  clientQuery: PostgresRuntimeEventRepositoryExecutor['query'],
): PostgresRuntimeEventRepositoryExecutor {
  return {
    query: <T extends QueryResultRow>(text: string, values?: readonly unknown[]) => clientQuery<T>(text, values),
  };
}

function failedStep(name: string, errorKind: string): PostgresRuntimeEventWriteSmokeStep {
  return { name, ok: false, errorKind };
}

async function runWritePath(
  executor: PostgresRuntimeEventRepositoryExecutor,
): Promise<PostgresRuntimeEventWriteSmokeStep[]> {
  const steps: PostgresRuntimeEventWriteSmokeStep[] = [];
  ...
  if (appendA.ok === false) return [...steps, failedStep('appendEventA', appendA.error.kind)];
  if (appendA.value.seq !== 1) return [...steps, failedStep('appendEventA', `seq_expected_1_got_${appendA.value.seq}`)];
  ...
  if (appendB.ok === false) return [...steps, failedStep('appendEventB', appendB.error.kind)];
  if (appendB.value.seq !== 2) return [...steps, failedStep('appendEventB', `seq_expected_2_got_${appendB.value.seq}`)];
  ...
  if (appendC.ok === false) return [...steps, failedStep('appendEventC', appendC.error.kind)];
  if (appendC.value.seq !== 3) return [...steps, failedStep('appendEventC', `seq_expected_3_got_${appendC.value.seq}`)];
  ...
  if (appendCRetry.ok === false) return [...steps, failedStep('appendEventCIdempotentRetry', appendCRetry.error.kind)];
  ...
  if (eventById.ok === false) return [...steps, failedStep('getRuntimeEventById', eventById.error.kind)];
  ...
  if (listFrom0.ok === false) return [...steps, failedStep('listRuntimeEventsAfterSeq0', listFrom0.error.kind)];
  ...
  if (listFrom1.ok === false) return [...steps, failedStep('listRuntimeEventsAfterSeq1', listFrom1.error.kind)];
  ...
  if (ended.ok === false) return [...steps, failedStep('endRuntimeSession', ended.error.kind)];
  ...
  if (archived.ok === false) return [...steps, failedStep('archiveRuntimeSession', archived.error.kind)];
  ...
  if (restored.ok === false) return [...steps, failedStep('restoreRuntimeSession', restored.error.kind)];
...
export async function runPostgresRuntimeEventRepositoryRollbackWriteSmoke(): Promise<PostgresRuntimeEventWriteSmokeResult> {
