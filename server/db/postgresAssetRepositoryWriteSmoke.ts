...
import {
  createPostgresAssetRepository,
  type PostgresAssetRepositoryExecutor,
} from '../adapters/postgresAssetRepository.js';
...
/**
 * Rollback-only Asset write smoke (P5.13C). Inside a single transaction it creates
 * a deterministic smoke OWNER user + smoke campaign (reusing PostgresUserRepository
 * / PostgresCampaignRepository with the same client executor), then a storage ref
 * and an asset linked to both, exercises read/list/update/archive/restore on both
 * asset and storage ref, and ALWAYS rolls back. No permanent rows, no commit path,
 * no raw DB error or connection string in the output.
 */

export type PostgresAssetWriteSmokeStatus =
  | 'not_configured'
  | 'unreachable'
  | 'user_schema_missing'
...
export interface PostgresAssetWriteSmokeStep {
  name: string;
  ok: boolean;
  errorKind?: string;
}

export interface PostgresAssetWriteSmokeResult {
  status: PostgresAssetWriteSmokeStatus;
  ...
  steps: PostgresAssetWriteSmokeStep[];
  errorKind?: string;
}
...
function makeClientExecutor(
  clientQuery: PostgresAssetRepositoryExecutor['query'],
): PostgresAssetRepositoryExecutor {
  return {
    query: <T extends QueryResultRow>(text: string, values?: readonly unknown[]) => clientQuery<T>(text, values),
  };
}

function failedStep(name: string, errorKind: string): PostgresAssetWriteSmokeStep {
  return { name, ok: false, errorKind };
}

async function runWritePath(
  executor: PostgresAssetRepositoryExecutor,
): Promise<PostgresAssetWriteSmokeStep[]> {
  const steps: PostgresAssetWriteSmokeStep[] = [];
  ...
  if (createdRef.ok === false) return [...steps, failedStep('createStorageRef', createdRef.error.kind)];
  steps.push({ name: 'createStorageRef', ok: true });

  const refById = await assetRepository.getStorageRefById(SMOKE_STORAGE_REF_ID);
  if (refById.ok === false) return [...steps, failedStep('getStorageRefById', refById.error.kind)];
  ...
  if (updatedRef.ok === false) return [...steps, failedStep('updateStorageRef', updatedRef.error.kind)];
  ...
  if (createdAsset.ok === false) return [...steps, failedStep('createAsset', createdAsset.error.kind)];
  steps.push({ name: 'createAsset', ok: true });

  const assetById = await assetRepository.getAssetById(SMOKE_ASSET_ID);
  if (assetById.ok === false) return [...steps, failedStep('getAssetById', assetById.error.kind)];
  ...
  if (byOwner.ok === false) return [...steps, failedStep('listAssetsByOwner', byOwner.error.kind)];
  ...
  if (byCampaign.ok === false) return [...steps, failedStep('listAssetsByCampaign', byCampaign.error.kind)];
  ...
  if (updatedAsset.ok === false) return [...steps, failedStep('updateAsset', updatedAsset.error.kind)];
  ...
  if (archivedAsset.ok === false) return [...steps, failedStep('archiveAsset', archivedAsset.error.kind)];
  ...
  if (restoredAsset.ok === false) return [...steps, failedStep('restoreAsset', restoredAsset.error.kind)];
  ...
  if (archivedRef.ok === false) return [...steps, failedStep('archiveStorageRef', archivedRef.error.kind)];
  ...
  if (restoredRef.ok === false) return [...steps, failedStep('restoreStorageRef', restoredRef.error.kind)];
...
export async function runPostgresAssetRepositoryRollbackWriteSmoke(): Promise<PostgresAssetWriteSmokeResult> {
  ...
  if (schema.status !== 'ready') {
    const status: PostgresAssetWriteSmokeStatus =
