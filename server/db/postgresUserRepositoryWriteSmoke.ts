...
import {
  ...
  type PostgresUserRepositoryExecutor,
...
export type PostgresUserRepositoryWriteSmokeStatus =
  | 'not_configured'
  | 'unreachable'
  | 'schema_missing'
  | 'rolled_back'
...
export interface PostgresUserRepositoryWriteSmokeStep {
  name: string;
  ok: boolean;
  errorKind?: string;
}

export interface PostgresUserRepositoryWriteSmokeResult {
  status: PostgresUserRepositoryWriteSmokeStatus;
  ...
  steps: PostgresUserRepositoryWriteSmokeStep[];
  errorKind?: string;
}
...
function makeClientExecutor(clientQuery: PostgresUserRepositoryExecutor['query']): PostgresUserRepositoryExecutor {
  return {
    query: <T extends QueryResultRow>(text: string, values?: readonly unknown[]) =>
      clientQuery<T>(text, values),
  };
}

function failedStep(name: string, errorKind: string): PostgresUserRepositoryWriteSmokeStep {
  return { name, ok: false, errorKind };
}

async function runRepositoryWritePath(
  repository: PostgresUserRepository,
): Promise<PostgresUserRepositoryWriteSmokeStep[]> {
  const steps: PostgresUserRepositoryWriteSmokeStep[] = [];
  ...
  if (createResult.ok === false) return [failedStep('createUserWithIdentity', createResult.error.kind)];
  steps.push({ name: 'createUserWithIdentity', ok: true });

  const byId = await repository.getUserById(WRITE_SMOKE_USER_ID);
  if (byId.ok === false) return [...steps, failedStep('getUserById', byId.error.kind)];
  ...
  const byIdentity = await repository.getUserByIdentity(
    WRITE_SMOKE_PROVIDER_KIND,
    WRITE_SMOKE_PROVIDER_SUBJECT,
  );
  if (byIdentity.ok === false) return [...steps, failedStep('getUserByIdentity', byIdentity.error.kind)];
  ...
  if (profile.ok === false) return [...steps, failedStep('getUserProfile', profile.error.kind)];
  ...
  if (saveProfile.ok === false) return [...steps, failedStep('saveUserProfile', saveProfile.error.kind)];
  steps.push({ name: 'saveUserProfile', ok: true });

  const updatedRead = await repository.getUserProfile(WRITE_SMOKE_USER_ID);
  if (updatedRead.ok === false) return [...steps, failedStep('getUserProfileAfterSave', updatedRead.error.kind)];
...
export async function runPostgresUserRepositoryRollbackWriteSmoke(): Promise<PostgresUserRepositoryWriteSmokeResult> {
