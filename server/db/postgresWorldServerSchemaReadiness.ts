...
/**
 * World Server + Membership + Game System schema readiness (P5.16-P5.18B) —
 * read-only, server-only.
 *
 * Mirrors the other readiness helpers. World server tables reference users and
 * campaigns, so it reuses the Campaign readiness helper (which already chains the
 * User check), then verifies all seven world-server tables + required columns. Never
 * prints the connection string, never creates tables, never runs a migration.
 */

export type PostgresWorldServerSchemaReadinessStatus =
  | 'not_configured'
  | 'unreachable'
  | 'user_schema_missing'
...
export interface PostgresWorldServerSchemaReadinessResult {
  status: PostgresWorldServerSchemaReadinessStatus;
...
function classifySchemaError(error: unknown): Pick<PostgresWorldServerSchemaReadinessResult, 'status' | 'errorKind'> {
...
export async function checkPostgresWorldServerSchemaReadiness(): Promise<PostgresWorldServerSchemaReadinessResult> {
...
export function getRequiredPostgresWorldServerSchemaColumns(): Record<string, string[]> {
  const out: Record<string, string[]> = {};
  for (const table of WORLD_TABLES) out[table] = [...REQUIRED_COLUMNS[table]];
  return out;
}
