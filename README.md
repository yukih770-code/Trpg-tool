...
- 内生 AI v1：后端模型网关、Ollama 本地 provider、真实可用性检查，以及 DND Builder 的结构化建议、确定性预览、确认写入、本地审计和安全撤销。

## 尚未完成

- DND Beyond 级别的完整车卡与升级体验；COC / CP RED 的完整纵向闭环。
- 正式账号安全、对象存储、公开发布审核、订阅/安装/更新与作者生态。
- 战役 / Runtime AI 循环、云端模型 provider、用量计费，以及跨任务编排与预算治理。
...
当前 v1 可连接部署者自行运行的 Ollama。项目不捆绑模型，也不会假装模型已安装；未配置时车卡助手会显示真实不可用状态。配置示例见 `.env.example` 中的 `LOCAL_AI_*` 项。
