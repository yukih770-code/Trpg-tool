...
本地 AI 不是向用户出售的独立服务，而是深入平台内部的“智能执行层”：辅助车卡、备团、运行时主持、资料创作和其他项目工作。未来本地模型与付费云端 API 通过同一模型网关提供计算，但 AI 只能产生建议、草稿或待确认意图；确定性规则、权限、校验和持久化仍由平台服务负责。
