...
- 内生 AI v1：认证后的模型目录与设备级 `关闭 / 自动 / 指定本地模型` 设置，自动路由优先已安装 Qwen 3.6；DND Builder 提供结构化建议、确定性预览、确认写入与安全撤销；活跃主持人可在 Runtime 用投影日志生成 Session 草稿并明确选择私密/公开追加。
...
当前 v1 可连接部署者自行运行的 Ollama。项目不捆绑模型，也不会假装模型已安装；设置页只展示后端安全投影的已安装模型，不暴露 provider URL。用户选择只改变下一次 AI 请求的计算路由，不扩大权限或绕过确认。云端 API 入口明确保持不可用。配置示例见 `.env.example` 中的 `LOCAL_AI_*` 项。
