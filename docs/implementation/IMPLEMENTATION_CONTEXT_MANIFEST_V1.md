...
- **Source compendium entries are immutable** — a server may enable, disable, or
  select a separately versioned replacement for new work, but never delete/mutate
  owner-source content or re-resolve an existing character/campaign/runtime snapshot.
...
| Server content settings / Compendium / Workshop authoring | Index → Server Content Sets; `SERVER_CONTENT_SET_RESOLUTION_V1`, `SERVER_RULESET_VERSIONING_SOFT_UPDATE_UX_V1`, `P6_PRIVATE_COMPENDIUM_PACK_PUBLISH`, `src/lib/platform/serverContentSet.ts`, `server/api/privateCompendiumPackApiHandlers.ts`, `src/components/platform/DndPrivateSpeciesPackEditorPanel.tsx`, and migration `0009_remaining_platform_foundation.sql`. Content source packs remain immutable; owner/admin publication creates a separate server-private pack/version/entry set atomically. The first species authoring form saves metadata only; it does not yet affect Creator or Runtime. No direct source mutation, display-name matching, public Workshop publishing, or retroactive character/runtime rewrites. |
...
| Token Inspect Safe Visual Surface (P5.TOKEN-INSPECT-SAFE-VISUAL-SURFACE) | `P5_TOKEN_INSPECT_SAFE_VISUAL_SURFACE`; `server/room/roomRuntimeVisibilityProjection.ts`, Room HTTP/WebSocket projection hooks, `roomRuntimeVisibility.ts`, `RuntimeTokenInspectPanel.tsx`, `BasicMapBoard.tsx` (`runtime:verify:runtime-visibility-projection`, `frontend:verify:token-inspect-safe-visual-surface`). Server projects map/runtime views before transport: host full, owner exact own data, player/spectator public observed data, hidden Tokens absent; no investigation resolution, new RBAC, DB migration, or token-authority change. |
| Character Clearance Details + Token Control Fix | `P5_CHARACTER_CLEARANCE_DETAILS_AND_TOKEN_CONTROL_FIX`; `characterClearanceDetails.ts`, `CharacterClearanceDetailsPanel.tsx`, `roomTokenOwnership.ts`, Room binding transport, and existing server guards (`frontend:verify:character-clearance-details`, `frontend:verify:player-token-control`). Bounded optional review metadata only; old shallow bindings remain valid. Player movement requires verified viewer, exact member/binding metadata, an approved clearance binding, and a supported persisted Token source; no full rules engine or character synchronization. |
...
Read `P5_CHARACTER_CLEARANCE_DETAILS_AND_TOKEN_CONTROL_FIX.md` before changing
Lobby review details or player Token movement. The optional details payload is
bounded review metadata, not a legality engine. Exact member plus binding
metadata remains mandatory for player movement; old metadata-free Tokens replay
but are host-controlled.
