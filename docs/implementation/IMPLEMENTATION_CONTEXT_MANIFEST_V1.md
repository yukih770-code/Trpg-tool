...
| Cloud Private Alpha Readiness (P5.CLOUD-PRIVATE-ALPHA-READINESS) | Index → Cloud Private Alpha Readiness; `docs/deployment/CLOUD_PRIVATE_ALPHA_DEPLOYMENT.md`, `docs/deployment/PRIVATE_ALPHA_AUTH_PLAN.md`; `server/config/serverRuntimeConfig.ts`, `cloudPrivateAlphaConfigSmoke.ts`, `server/room-server.ts`, `.env.cloud.*.example`; explicit `localDev` / `cloudPrivateAlpha` / `production` contract, cloud startup fails closed without backend DB/public URLs/explicit CORS or when dev auth is requested, frontend fails clearly for missing non-dev API URL; no cloud deployment, OAuth, DB migration, or protocol change |
