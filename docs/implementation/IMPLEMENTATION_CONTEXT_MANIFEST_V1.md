...
| Server content settings / Compendium / Workshop authoring | Index → Server Content Sets; `SERVER_CONTENT_SET_RESOLUTION_V1`, `SERVER_RULESET_VERSIONING_SOFT_UPDATE_UX_V1`, `P6_PRIVATE_COMPENDIUM_PACK_PUBLISH`, `src/lib/platform/serverContentSet.ts`, `server/api/privateCompendiumPackApiHandlers.ts`, and migration `0009_remaining_platform_foundation.sql`. Content source packs remain immutable; owner/admin publication creates a separate server-private pack/version/entry set atomically. No direct source mutation, display-name matching, public Workshop publishing, or retroactive character/runtime rewrites. |
