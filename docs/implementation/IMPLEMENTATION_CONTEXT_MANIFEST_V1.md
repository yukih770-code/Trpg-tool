...
| Server content settings / Compendium / Workshop authoring | Index → Server Content Sets; `SERVER_CONTENT_SET_RESOLUTION_V1`, `SERVER_RULESET_VERSIONING_SOFT_UPDATE_UX_V1`, `src/lib/platform/serverContentSet.ts`, and migration `0009_remaining_platform_foundation.sql`. Content source packs remain immutable; server drafts choose enabled packs plus entry policies; custom replacements receive separate ids/revisions. No direct source mutation, display-name matching, or retroactive character/runtime rewrites. |
