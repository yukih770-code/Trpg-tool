...
DND Equipment Local CHM Completion v1 completed the structured identity/catalog pass for the 38 weapon rows in `玩家手册2024/装备/武器.htm` and 13 armor/shield rows in `玩家手册2024/装备/护甲.htm`. The catalog now carries local-source references, aliases needed by existing starter equipment, and display-only weapon mastery labels. Tools, adventuring gear, packs, and all executable equipment mechanics remain deferred; no inventory behavior, AC calculation, attacks, ammunition, or Runtime behavior changed.
...
| Equipment | 55 structured core rows (38 weapons + 13 armor/shields + 4 legacy gear samples) | 13 category rows | 13 PHB equipment files with many table rows | weapons/armor complete; other categories incomplete | Extract tools, adventuring gear, packs, mounts, services, and magic-item rows separately |
