...
| Data management MVP, local stores, backup, import, export, restore, local personal campaign workflow | `DATA_MANAGEMENT_MVP_SCOPE_V1.md` | Triggered only for genuinely local/offline data-management work. Its §1/§3 exclusion lists are a **superseded 2026 phase note** — backend, multiplayer, permissions, `CampaignMembership`, `CampaignActorInstance` and map/Scene/Token now exist and are governed by their own Level 1 contracts, so this document must not be used to block them. Still binding: keep local-first work replaceable behind repository/adapter boundaries, validate before write, and never present placeholder behavior as real data management. |
