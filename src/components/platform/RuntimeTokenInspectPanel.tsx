...
export function RuntimeTokenInspectPanel({ token, combatant, role, onClose }: Props) {
  ...
  return <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-950/25 p-4" role="presentation" onPointerDown={onClose}>
    <section role="dialog" aria-modal="true" aria-label={`${token.displayName ?? token.name} 的查看信息`} onPointerDown={(event) => event.stopPropagation()} className="w-full max-w-sm rounded-xl border border-slate-300 bg-white p-4 shadow-2xl">
      ...
      <div className="mt-3 rounded-lg bg-slate-50 px-3 py-2.5 text-[11px] leading-5 text-slate-600">
