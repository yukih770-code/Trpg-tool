...
function ArtifactBody({ artifact, labels }: { artifact: SavedCampaignArtifact; labels: ReturnType<typeof copy> }) {
  ...
  return <div className="mt-3 space-y-3 text-sm">
    ...
    {(artifact.task === 'session_character_biography' || artifact.task === 'session_quest_log') && <p className="rounded-lg bg-violet-500/10 px-3 py-2 text-xs font-bold text-violet-900">{labels.sessionOutcome} · {labels.sessionWarning}</p>}
...
export function CampaignAiArtifactPanel({ locale, worldServerId, campaignId, canManage }: Props) {
  ...
  const [task, setTask] = useState<CampaignArtifactGenerationTask>('preparation_brief');
  const [focus, setFocus] = useState('');
  const [families, setFamilies] = useState<CampaignArtifactSourceFamily[]>(['campaign_summary', 'actor_summaries', 'room_summaries', 'adopted_memories']);
  ...
  async function mutateAdoption(artifact: SavedCampaignArtifact) {
    setBusy('mutate'); setError(null);
    try {
      const next = artifact.adoption?.status === 'active'
        ? await campaignArtifactAssistantApiClient.withdrawAdoption(worldServerId, campaignId, artifact.artifactId)
        : await campaignArtifactAssistantApiClient.adopt(worldServerId, campaignId, artifact.artifactId);
      setArtifacts((items) => items.map((item) => item.artifactId === next.artifactId ? next : item));
    } catch (reason) { setError(errorText(reason, labels)); }
    finally { setBusy(null); }
  }
    ...
    <div className="mt-4 border-t border-[#2f2a22]/10 pt-4">
      ...
      {busy === 'load' ? <p className="text-sm text-[#51483d]">…</p> : !available ? <div className="rounded-xl bg-white/65 p-3 text-sm text-[#6c3c32]"><p>{labels.unavailable}</p><button type="button" onClick={() => void load()} className="mt-2 text-xs font-bold underline">{labels.refresh}</button></div> : <>
        ...
        {!draft && <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(260px,.8fr)]">
          <div><div className="flex flex-wrap gap-2">{(['preparation_brief', 'campaign_recap', 'worldbuilding_outline', 'adventure_seed'] as CampaignArtifactGenerationTask[]).map((value) => <button key={value} type="button" onClick={() => setTask(value)} className={`rounded-lg px-3 py-2 text-xs font-bold ${task === value ? 'bg-[#2b241d] text-white' : 'bg-white/75 text-[#40382f]'}`}>{labels[value]}</button>)}</div>
