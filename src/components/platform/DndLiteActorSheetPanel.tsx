...
export function DndLiteActorSheetPanel({ locale, canManage, campaignActors, sheets, onSave, onClear, onUseAction, onAddToCombat, onReviewSource, onAcceptSource }: Props) {
    ...
    <section className="mt-5 rounded-2xl border border-[#58180d]/15 bg-[#fffaf0] p-4 shadow-sm">
      ...
      {campaignActors.length === 0 ? <p className="mt-4 rounded-xl border border-dashed border-[#2f2a22]/15 bg-white p-4 text-sm text-[#51483d]">{t('dndActorSheet.noActors')}</p> : <>
        ...
        {selectedActor && <div className="mt-4 grid gap-3">
          ...
          <div className="rounded-xl border border-[#2f2a22]/10 bg-white p-3"><textarea value={draft.notes ?? ''} onChange={(event) => update({ notes: event.target.value || undefined })} disabled={!canManage || saving} placeholder={t('dndActorSheet.notes')} className="min-h-20 w-full rounded-md border border-[#2f2a22]/15 px-3 py-2 text-sm disabled:opacity-50" /><input value={(draft.tags ?? []).join(', ')} onChange={(event) => update({ tags: event.target.value.split(',').map((item) => item.trim()).filter(Boolean) })} disabled={!canManage || saving} placeholder={t('dndActorSheet.tags')} className="mt-2 w-full rounded-md border border-[#2f2a22]/15 px-3 py-2 text-sm disabled:opacity-50" /><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => void saveSheet()} disabled={!canManage || saving} className="rounded-md bg-[#17130f] px-3 py-2 text-xs font-bold text-white disabled:opacity-40">{t('dndActorSheet.save')}</button><button type="button" onClick={() => onAddToCombat(getDndLiteCombatantPrefill(draft, selectedActor.campaignActorInstanceId))} disabled={!canManage || saving} className="rounded-md border border-[#2f2a22]/15 bg-white px-3 py-2 text-xs font-bold disabled:opacity-40">{t('dndActorSheet.addToCombat')}</button><button type="button" onClick={() => void clearSheet()} disabled={!canManage || saving} className="rounded-md border border-[#2f2a22]/15 bg-white px-3 py-2 text-xs font-bold disabled:opacity-40">{t('dndActorSheet.clear')}</button></div>{notice && <p className="mt-2 text-xs text-[#51483d]">{notice}</p>}<p className="mt-3 text-[11px] text-[#51483d]">{summary.displayName} · AC {summary.armorClass ?? '—'} · HP {summary.hpText ?? '—'} · {summary.actionCount} {t('dndActorSheet.actionCount')}</p></div>
        </div>}
      </>}
