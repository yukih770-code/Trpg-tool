...
assert.ok(empty.includes('暂无可用攻击'));
assert.ok(empty.includes('已批准角色的装备与来源'));
assert.ok(panel.includes('待确认角色 → Sword → Target'));
const loading = renderToStaticMarkup(<RuntimeDndActionPanel scopeKey="loading" actorCombatantId="pc" actions={[]} actionsLoading />);
assert.ok(loading.includes('正在读取可用攻击'));
assert.ok(!loading.includes('暂无可用攻击'));
assert.equal(dndAttackErrorText(new RoomServerHttpError(409, 'dnd_attack_out_of_range')), '目标超出此攻击的近战范围。');
assert.equal(dndAttackErrorText(new RoomServerHttpError(400, 'unknown_action')), '该攻击已变更或不可用，请刷新动作后重新选择。');
console.log('T12 rendering smoke passed: structured attack details, projection-gated vitals, no hidden-value fallback, intent summary, loading and source-aware empty state.');
