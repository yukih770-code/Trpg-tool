...
export interface RuntimeCharacterSheetPanelProps {
  ...
  dndActionTargets?: Array<{ id: string; label: string }>;
  selectedDndActionTargetId?: string;
  onSelectDndActionTarget?: (targetId: string | undefined) => void;
        ...
        <div className="border-b border-slate-200 py-3">
          ...
          <div className="space-y-1.5">
            {dndActions.map((action) => {
              ...
              const target = dndActionTargets.find((candidate) => candidate.id === selectedDndActionTargetId);
              const targetSuffix = target ? ` → ${target.label}` : '';
              return (
                <div key={action.id} className="flex flex-wrap items-center justify-between gap-1.5 rounded border border-slate-200/80 bg-white/75 px-2 py-1.5">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {attack && (
                      <button type="button" disabled={!onRollDndAction} onClick={() => onRollDndAction?.({ expression: attack, label: `${summary.displayName} · ${action.name} 攻击${targetSuffix}` })} className="rounded border border-slate-400/45 bg-white px-2 py-1 text-[10px] font-bold text-slate-700 disabled:opacity-45">攻击 {attack}</button>
                    )}
                    {damage && (
                      <button type="button" disabled={!onRollDndAction} onClick={() => onRollDndAction?.({ expression: damage, label: `${summary.displayName} · ${action.name} 伤害${targetSuffix}` })} className="rounded border border-amber-500/40 bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-800 disabled:opacity-45">伤害 {damage}</button>
                    )}
          ...
          <p className="mt-1 text-[9px] leading-relaxed text-slate-500">目标只会附在服务器骰子日志中；命中、伤害结算与生命值修改仍由主持人手动确认。</p>
        </div>
      )}
