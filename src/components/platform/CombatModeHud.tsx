    ...
    <section className="mb-4 overflow-hidden rounded-xl border border-[#58180d]/30 bg-[#17130f] text-[#fff8e6] shadow-lg">
      ...
      {active && <div className="grid gap-3 border-t border-white/10 bg-black/15 px-3 py-3 md:grid-cols-[1fr_auto]">
        <div>
          ...
          <div className="mt-1 flex flex-wrap gap-2 text-xs text-[#fff8e6]/80">{hpLabel(active) && <span>{hpLabel(active)}</span>}{active.temporaryHp ? <span>{zh ? '临时 HP' : 'Temp HP'} {active.temporaryHp}</span> : null}{active.armorClass !== undefined ? <span>AC {active.armorClass}</span> : null}{active.conditions.length > 0 ? <span>{zh ? '状态：' : 'Conditions: '}{active.conditions.join('、')}</span> : <span>{zh ? '无状态' : 'No conditions'}</span>}</div>
