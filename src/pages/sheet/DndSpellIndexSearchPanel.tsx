...
import {
  getDndSpellIndexStats,
  searchDndSpellIndexEntries,
...
export function DndSpellIndexSearchPanel() {
  ...
  const stats = useMemo(() => getDndSpellIndexStats(), []);
  const results = useMemo(
    () => searchDndSpellIndexEntries(query, { level: levelFilter, scope: scopeFilter }),
    [query, levelFilter, scopeFilter],
  );
    ...
    <div className="rounded-lg border border-[#58180d]/20 bg-white/45 p-4 shadow-sm">
      ...
      >
        ...
        <span className="rounded-full bg-[#58180d]/8 px-2 py-0.5 text-[10px] text-[#2c1810]/55">
          只读索引 · {stats.total} 条 · 不可选择 / 不可施放 / 不写入角色
        </span>
      </button>
