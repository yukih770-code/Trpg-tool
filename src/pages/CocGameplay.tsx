      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="border border-[#059669]/30 bg-[#111] p-4 flex flex-col min-h-[300px]">
          ...
          <div className="flex-1 font-mono text-xs overflow-hidden custom-scrollbar max-h-[200px] overflow-y-auto mb-4 bg-black/40 p-2 border border-[#059669]/10">
             {combatLog.map((log, i) => (
               <div key={i} className="border-b border-[#059669]/10 py-1 last:border-0">{log}</div>
             ))}
          </div>
          
          <div className="mt-auto pt-2 border-t border-[#059669]/30">
             <div className="flex justify-between items-center mb-2">
               <div className="text-[10px] uppercase font-bold text-[#059669]">选取投掷骰: {Object.entries(diceTray).filter(([_, c]) => c > 0).map(([d, c]) => `${c}${d}`).join(' + ')}</div>
               <div className="flex gap-1">
             ...
             <div className="flex flex-wrap gap-2">
               {['d4', 'd6', 'd8', 'd10', 'd12', 'd20', 'd100'].map(die => (
                 <button key={die} 
                   className="w-10 h-10 border border-[#059669] bg-[#1a1a1a] text-[#059669] font-bold text-xs hover:bg-[#059669] hover:text-[#111] transition-colors relative"
