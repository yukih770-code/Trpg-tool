...
const cases: Array<{ name: string; run: () => void }> = [
  ...
  { name: 'campaign actor source projects combat summary without combatant id', run: () => { const projected = tokenWithCombatProjection(heroToken, [combatant]); assert(projected.hpSummary?.current === 12 && projected.conditionSummary?.[0] === 'Blessed', 'campaign actor source projection stale'); } },
