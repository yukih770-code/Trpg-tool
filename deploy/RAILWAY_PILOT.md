...
2. Create the app service from the audited GitHub revision. Keep one replica and application sleeping disabled. Attach an asset volume at `/data/assets` **before starting the app**. `railway.json` requires this mount and configures `/health` and a five-minute startup/pre-deploy timeout.
