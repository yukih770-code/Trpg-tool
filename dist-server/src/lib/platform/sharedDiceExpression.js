 ...
 */
export const ALLOWED_DICE_SIDES = [4, 6, 8, 10, 12, 20, 100];
const ALLOWED_SIDES_SET = new Set(ALLOWED_DICE_SIDES);
...
export function parseSharedDiceExpression(raw) {
    ...
    for (const m of tokens) {
        ...
        if (token.includes('d')) {
            if (sign === '-')
                return { ok: false, code: 'invalidExpression', message: 'Negative dice terms are not supported.' };
            ...
            if (!ALLOWED_SIDES_SET.has(sides))
                return { ok: false, code: 'invalidExpression', message: `Unsupported die d${sides}.` };
            ...
            if (totalDice > MAX_TOTAL_DICE)
                return { ok: false, code: 'expressionTooLarge', message: `Too many dice (max ${MAX_TOTAL_DICE}).` };
    ...
    if (terms.length === 0)
        return { ok: false, code: 'invalidExpression', message: 'At least one dice term is required.' };
    ...
    const normalizedExpression = terms.map((t) => `${t.count}d${t.sides}`).join('+') +
...
export function formatSharedDiceRoll(roll) {
