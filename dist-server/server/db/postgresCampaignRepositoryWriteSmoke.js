...
async function runWritePath(executor) {
    ...
    if (created.ok === false)
        return [...steps, failedStep('createCampaign', created.error.kind)];
    steps.push({ name: 'createCampaign', ok: true });
    const byId = await campaignRepository.getCampaignById(SMOKE_CAMPAIGN_ID);
    if (byId.ok === false)
        return [...steps, failedStep('getCampaignById', byId.error.kind)];
    ...
    if (listed.ok === false)
        return [...steps, failedStep('listCampaignsByOwner', listed.error.kind)];
    ...
    if (updated.ok === false)
        return [...steps, failedStep('updateCampaign', updated.error.kind)];
    ...
    if (archived.ok === false)
        return [...steps, failedStep('archiveCampaign', archived.error.kind)];
    ...
    if (restored.ok === false)
        return [...steps, failedStep('restoreCampaign', restored.error.kind)];
...
export async function runPostgresCampaignRepositoryRollbackWriteSmoke() {
