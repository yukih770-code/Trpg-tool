...
async function runWritePath(executor) {
    ...
    if (appendEvent.ok === false)
        return [...steps, failedStep('appendRuntimeEvent', appendEvent.error.kind)];
    ...
    if (createdArtifact.ok === false)
        return [...steps, failedStep('createGeneratedArtifact', createdArtifact.error.kind)];
    steps.push({ name: 'createGeneratedArtifact', ok: true });
    const artifactById = await repo.getGeneratedArtifactById(SMOKE_ARTIFACT_ID);
    if (artifactById.ok === false)
        return [...steps, failedStep('getGeneratedArtifactById', artifactById.error.kind)];
    ...
    if (artifactsByOwner.ok === false)
        return [...steps, failedStep('listGeneratedArtifactsByOwner', artifactsByOwner.error.kind)];
    ...
    if (artifactsByCampaign.ok === false)
        return [...steps, failedStep('listGeneratedArtifactsByCampaign', artifactsByCampaign.error.kind)];
    ...
    if (artifactsBySession.ok === false)
        return [...steps, failedStep('listGeneratedArtifactsByRuntimeSession', artifactsBySession.error.kind)];
    ...
    if (updatedArtifact.ok === false)
        return [...steps, failedStep('updateGeneratedArtifact', updatedArtifact.error.kind)];
    ...
    if (createdMemory.ok === false)
        return [...steps, failedStep('createAiMemoryEntry', createdMemory.error.kind)];
    steps.push({ name: 'createAiMemoryEntry', ok: true });
    const memoryById = await repo.getAiMemoryEntryById(SMOKE_MEMORY_ID);
    if (memoryById.ok === false)
        return [...steps, failedStep('getAiMemoryEntryById', memoryById.error.kind)];
    ...
    if (memoryByOwner.ok === false)
        return [...steps, failedStep('listAiMemoryEntriesByOwner', memoryByOwner.error.kind)];
    ...
    if (memoryByCampaign.ok === false)
        return [...steps, failedStep('listAiMemoryEntriesByCampaign', memoryByCampaign.error.kind)];
    ...
    if (memoryBySession.ok === false)
        return [...steps, failedStep('listAiMemoryEntriesByRuntimeSession', memoryBySession.error.kind)];
    ...
    if (updatedMemory.ok === false)
        return [...steps, failedStep('updateAiMemoryEntry', updatedMemory.error.kind)];
    ...
    if (artifactSource.ok === false)
        return [...steps, failedStep('createAiContextSourceArtifact', artifactSource.error.kind)];
    ...
    if (memorySource.ok === false)
        return [...steps, failedStep('createAiContextSourceMemory', memorySource.error.kind)];
    ...
    if (sourcesForArtifact.ok === false)
        return [...steps, failedStep('listAiContextSourcesForArtifact', sourcesForArtifact.error.kind)];
    ...
    if (sourcesForMemory.ok === false)
        return [...steps, failedStep('listAiContextSourcesForMemoryEntry', sourcesForMemory.error.kind)];
    ...
    if (archivedArtifact.ok === false)
        return [...steps, failedStep('archiveGeneratedArtifact', archivedArtifact.error.kind)];
    ...
    if (restoredArtifact.ok === false)
        return [...steps, failedStep('restoreGeneratedArtifact', restoredArtifact.error.kind)];
    ...
    if (archivedMemory.ok === false)
        return [...steps, failedStep('archiveAiMemoryEntry', archivedMemory.error.kind)];
    ...
    if (restoredMemory.ok === false)
        return [...steps, failedStep('restoreAiMemoryEntry', restoredMemory.error.kind)];
...
export async function runPostgresGeneratedArtifactRepositoryRollbackWriteSmoke() {
