...
function toShowcaseSlots(value) {
    if (!Array.isArray(value))
        return [];
...
function toSectionVisibility(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return {};
    return value;
}
function toProviderKind(value) {
...
function rowToProfile(row) {
    if (!row.profile_handle || !row.profile_display_name)
        return undefined;
    return {
        userId: row.user_id,
...
`;
function makeDefaultHandle(userId) {
    return `local-${userId.replace(/[^a-zA-Z0-9]/g, '').slice(0, 12).toLowerCase()}`;
}
async function insertUserProfile(executor, profile, now) {
...
export function createPostgresUserRepository(executor = defaultExecutor, options = {}) {
    const useInternalTransactions = options.useInternalTransactions ?? true;
    async function getUserById(userId) {
        try {
            const result = await executor.query(`${USER_RECORD_SELECT} WHERE u.user_id = $1 LIMIT 1`, [userId]);
    ...
    async function getUserByIdentity(providerKind, providerSubject) {
        try {
    ...
    async function createUserWithIdentity(input) {
        ...
        try {
            const write = async (writeExecutor) => {
...
export async function getUserByIdentity(providerKind, providerSubject) {
...
export async function checkPostgresUserRepositoryReadiness() {
