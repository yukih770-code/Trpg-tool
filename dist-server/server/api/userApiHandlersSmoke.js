...
export function createFakeUserRepository(config = {}) {
    const err = () => ({ ok: false, error: { kind: config.readError.kind, message: 'fake repository error', retryable: config.readError?.retryable } });
    return {
        ...
        async createUserWithIdentity(input) {
            return ok({ identity: input.identity, profile: input.profile });
        },
...
function evaluate(name, response, expected) {
    const actual = {
        ok: response.ok,
        statusCode: response.statusCode,
        errorKind: response.ok ? undefined : response.error.kind,
    };
    const pass = actual.ok === expected.ok &&
...
export async function runUserApiHandlersSmoke() {
    const cases = [];
