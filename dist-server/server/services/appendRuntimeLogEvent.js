...
const VALID_VISIBILITIES = ['public', 'hostOnly', 'actorPrivate'];
export function appendRuntimeLogEvent(roomRegistry, logRegistry, input) {
    ...
    const authorMember = input.authorMemberId !== undefined
        ? room.members.find((m) => m.memberId === input.authorMemberId)
        : undefined;
    if (input.authorMemberId !== undefined) {
        if (!authorMember)
            return { decision: 'memberNotFound', message: `No member "${input.authorMemberId}".` };
