...
const env = { ...process.env, ...dotenv.parse(await readFile('.env')),
  ...
  PORT: '8787', ROOM_SERVER_ENV: 'localDev', SERVER_RUNTIME_MODE: 'local',
  APP_PUBLIC_HTTP_URL: 'http://localhost:8787', APP_PUBLIC_WS_URL: 'ws://localhost:8787',
...
async function startBackend() {
  ...
  for (let i=0;i<100;i++) {
    try { const r=await fetch('http://localhost:8787/health'); const h=await r.json(); if(h.ok){console.log(JSON.stringify({status:'ready', backendPid:backend.pid, database:'isolated PostgreSQL 18', durableDirectory:dir, schemaCount:h.readiness?.readySchemaCount}));return;} } catch {}
